...
所有 Hx 组件使用 `data-hx-*` 属性进行样式和状态管理（非 CSS-in-JS）。这些属性是**保留的**——你**不能**将它们作为自定义 `data-hx-*` props 传递给组件，它们会被静默丢弃。
...
| `data-hx-text-uppercase` | 文本大写变换 | `""` 或不存在 |
...
| 属性 | 用途 | 取值 |
|------|------|------|
| `data-hx-temporary-display` | 内部：临时展示状态 | `""` |
