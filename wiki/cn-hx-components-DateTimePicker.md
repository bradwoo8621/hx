...
> **注意：** 伊斯兰历的三个变体（表格式、民用、Umalqura）的年/月**移动操作**（`moveYear`/`moveMonth`）通过共享的 `DateMoveAnyMonthsProvider` 超类实现（与印度历、波斯历相同的 12 个月机制）；目前仅年份面板尚未实现。
...
- **伊斯兰历（表格式 / 民用 / Umalqura）** — 月份面板已实现；伊斯兰历范围 [−640, 9666] 两端均为不完整年，因此 −640 年 1-4 月标记为 `bc`，9666 年 5-12 月标记为 `y10k`。年份面板尚未实现。
