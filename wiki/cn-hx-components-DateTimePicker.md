...
- **日本和历** — 月份面板已实现（年份面板尚未实现）；月份按年号制显示：当月首日年号与前一月不同时显示该年号（1 月恒显示年首年号），并列出月内出现的其他年号——例如 1989/01（1 月 7 日昭和→平成）与 1926/12（12 月 25 日大正→昭和）。唯一三个月内出现三个年号的月份为 1387/8（元中/至徳/嘉慶，南北朝重叠），显式处理；1582/10 短月（21 天，跳过 5-14 日）经由共享的 Gregorian-and-Julian helper 处理。
