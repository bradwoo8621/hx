...
| `valueSyncMode` | `'default' \| 'immediate'` | `'default'` | `'default'` 时面板编辑为草稿，点击日期或确定后提交；`'immediate'` 时每次编辑立即同步到模型 |
...
解析为公历的语言仍会遵循各自的语言习惯来显示弹窗的星期表头、星期名与日期顺序。周起始遵循 ICU 周数据（`Intl.Locale.getWeekInfo`）：

| 语言 | 文字 | 周起始 | 说明 |
|------|------|--------|------|
| `vi-VN`（越南语） | 拉丁字母 | 周一（Thứ 2） | 星期表头使用窄格式标签 |
| `ms-MY`（马来语） | 拉丁字母 | 周一（Isnin） | |
| `id-ID`（印尼语） | 拉丁字母 | 周日（Minggu） | |
| `lo-LA`（老挝语） | 老挝文 | 周日 | 老挝官方使用佛历（ພ.ສ.），但佛历 provider 仅覆盖 `th-*`，故 `lo-LA` 解析为公历 |
| `my-MM`（缅甸语） | 缅甸文 | 周日（တနင်္ဂနွေ） | 缅甸官方使用缅历，但无 provider 覆盖，故 `my-MM` 解析为公历 |
| `mn-MN`（蒙古语） | 西里尔字母 | 周一（Даваа） | |
| `de-DE`（德语） | 拉丁字母 | 周一（Montag） | 带点序数日（10. Juni） |
| `nl-NL`（荷兰语） | 拉丁字母 | 周一（maandag） | 月份名小写 |
| `fr-FR`（法语） | 拉丁字母 | 周一（lundi） | 窄格式星期标签（L, M, M, J, V, S, D） |
| `ru-RU`（俄语） | 西里尔字母 | 周一（понедельник） | 属格月份；年份带「г.」 |
| `es-ES`（西班牙语） | 拉丁字母 | 周一（lunes） | 「de」连接词（10 de junio de 2025） |
| `pt-BR`（巴西葡萄牙语） | 拉丁字母 | 周日（domingo） | 月份名小写 |
| `it-IT`（意大利语） | 拉丁字母 | 周一（lunedì） | 月份名小写 |
| `tr-TR`（土耳其语） | 拉丁字母 | 周一（Pazartesi） | 日期在前、星期在后（10 Haziran 2025 Salı） |
| `pl-PL`（波兰语） | 拉丁字母 | 周一（poniedziałek） | 属格月份；窄格式星期标签（p, w, ś, c, p, s, n） |
| `uk-UA`（乌克兰语） | 西里尔字母 | 周一（понеділок） | 属格月份；年份带「р.」 |
| `en-US`（英语，美国） | 拉丁字母 | 周日 | 月前日顺序（June 10, 2025） |
| `en-GB`（英语，英国） | 拉丁字母 | 周一 | 日前月顺序（10 June 2025） |
...
当模式含时间部分时，弹出面板在日期面板与底部按钮区之间显示时间输入行：整数输入框的数量与显示格式包含的时间字段一致（`h` 时、`n` 分、`s` 秒——例如 `:h`、`:hn` 或 `:hns`；时/分/秒分别使用 `@iu23z` / `@iu59z` / `@iu59z`），由静态冒号连接，样式上呈现为一个整体控件，另附三个时间快捷按钮：**开始**（`00:00:00`）、**正午**（`12:00:00`）、**结束**（`23:59:59`）。
...
默认（`valueSyncMode: 'default'`）下面板编辑为**草稿**：选日、年/月导航、今天按钮与时间编辑都只更新面板状态；值在用户点击日期或**确定**按钮时写入模型（两者都提交面板完整值并关闭面板）。`valueSyncMode: 'immediate'` 时每次编辑立即同步到模型，且不显示确定按钮。模式不含时间部分时，点击日期在两种模式下都立即提交。

**清除**按钮清空模型并关闭面板 —— 清除是终结操作，之后无法再提交草稿。

## 键盘导航

面板支持完整的键盘操作。面板打开时，真实焦点移动到面板容器，网格当前项以视觉高亮显示（与 select 选项的交互方式一致）；方向键移动高亮，Enter/Space 选中高亮项。鼠标悬停驱动同一高亮，鼠标下的元素与键盘目标始终保持同步。

- **Enter** — 打开弹出面板（`enterToOpenPopup` 为 `true` 时）；面板打开时选中高亮的网格单元格
- **Space** — 打开弹出面板（`spaceToOpenPopup` 为 `true` 时，默认启用）；面板打开时选中高亮的网格单元格
- **面板切换** — 头部月/年标签切换网格面板；选中月份（Enter/Space/点击）返回日面板，选中年份进入月面板，高亮自动重定位到新面板的状态单元格
- **Escape** — 关闭弹出面板，焦点返回输入框
- **方向键** — 在日/月/年网格中移动高亮（左/右 ±1 格，上/下 ±1 行；第一列向左回绕到上一行末列，最后一列向右到下一行首列；跳过 BC/10K 单元格；12 月向下到 13 月）
- **第一行向上** — 离开网格到头部月/年标签；**最后一行向下** — 进入时间行第一个输入框（存在时间部分时）或"今天"按钮
- **Tab / Shift+Tab** — 在面板区块间移动（头部 → 网格 → 时间行 → 底部）及区块内部；底部链尾 Tab 自然移出面板（与 select 过滤框一致），头部链首 Shift+Tab 返回输入框
- **时间输入框** — 输入数字（自动跳转下一字段：格式包含分/秒时依次跳转，否则跳到"一日之始"快捷按钮）；光标在开头时向左移到上一字段，在末尾时向右移到下一字段或快捷按钮
