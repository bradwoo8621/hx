...
const allowTouchMove = (el: EventTarget): boolean => {
	return locks.some(lock => lock.options.allowTouchMove != null && lock.options.allowTouchMove(el));
};
...
const preventDefault = (rawEvent: HandleScrollEvent): boolean => {
	...
	if (e.target != null && allowTouchMove(e.target)) {
		return true;
	}
...
const handleScroll = (event: HandleScrollEvent, targetElement: HTMLElement): boolean => {
	...
	if (event.target != null && allowTouchMove(event.target)) {
		return false;
	}
