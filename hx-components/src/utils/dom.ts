...
export class DOMUtils {
	...
	>(
		...
		return Object.keys(props).reduce((wrapped, key) => {
	 ...
	 */
	static isAttributeNameSafe(attributeName: string): boolean {
	 ...
	 */
	static interposeToChildren<P extends object>(interposition?: P, children?: ReactNode): ReactNode {
		if (interposition == null || children == null) {
			return children;
		}

		return DOMUtils.interposePropsToChildren((props) => {
			return {
				...interposition,
...
// noinspection JSUnusedGlobalSymbols
	 ...
	 */
	static forceInterposeToChildren<P extends object>(interposition?: P, children?: ReactNode): ReactNode {
	 ...
	 */
	static computeTransitionAndAnimation(el: HTMLElement) {
	 ...
	 */
	static computeGapToViewportEdges<R extends RectToGetGapsToEdge>(rect: R, gapToEdge: number): GapsToEdge<R> {
	 ...
	 */
	static intersectWithContainer(el: HTMLElement, container: HTMLElement): boolean {
	 ...
	 */
	static scrollIntoViewIfNeed(dom: HTMLElement | null | undefined, scrollOptions?: boolean | ScrollIntoViewOptions): boolean {
		if (dom == null) {
			return false;
		}
		...
		if (DOMUtils.intersectWithContainer(dom, container)) {
	 ...
	 */
	static handleFocusClickOfOthers(handler: (ev: Event) => void): (() => void) {
	 ...
	 * @param el
	 */
	static ancestorsOf(el: HTMLElement): Array<HTMLElement> {
	 ...
	 */
	static getScrollableElements(elements: Array<HTMLElement>): Array<HTMLElement> {
	 ...
	 */
	static handleScrollResizeOfAncestors(
		...
		const handler = (ev?: Event) => {
			...
			delayHandle = requestAnimationFrame(() => {
				...
				if (elRect.top <= -maxHiddenY
					...
					const ancestors = DOMUtils.ancestorsOf(el);
					const scrollableAncestors = DOMUtils.getScrollableElements(ancestors);
	...
	static safeOnTransitionEndOnce = (
